...
def test_email_finding_not_spoofable_when_reject() -> None:
    finding = EmailFinding(record=EmailRecord(domain="acme.example"), dmarc=DmarcStatus.REJECT)
...
def test_email_finding_rejects_non_record() -> None:
    with pytest.raises(ValueError):
        EmailFinding(record="acme.example", dmarc=DmarcStatus.NONE)  # type: ignore[arg-type]


def test_email_finding_rejects_non_dmarc() -> None:
    with pytest.raises(ValueError):
        EmailFinding(record=EmailRecord(domain="acme.example"), dmarc="none")  # type: ignore[arg-type]
